	...
	if(!discussion && !flags.includes('include') && !flags.includes('preview') && doc.namespace == '사용자') {
		...
		if(blockdata) {
			data = `
				<div style="border-width: 5px 1px 1px; border-style: solid; border-color: red gray gray; padding: 10px; margin-bottom: 10px;" onmouseover="this.style.borderTopColor=\'blue\';" onmouseout="this.style.borderTopColor=\'red\';">
					<span${ver('4.13.0') ? '' : ' style="font-size: 14pt;"'}>이 사용자는 차단되었습니다.${ver('4.18.0') ? ` (#${blockdata.id})` : ''}</span><br /><br />
		...
		if(doc.namespace == '사용자') {
			if(getperm('admin', doc.title)) {
				data = `
					<div style="border-width: 5px 1px 1px; border-style: solid; border-color: #a188f1 #bcbcbc #bcbcbc; padding: 10px; margin-bottom: 10px;" onmouseover="this.style.borderTopColor=\'#c6b7f6\';" onmouseout="this.style.borderTopColor=\'#a188f1\';">
						<span${ver('4.13.0') ? '' : ' style="font-size: 14pt;"'}>이 사용자는 특수 권한을 보유하고 있습니다.</span>
					</div>
				` + data;
				}
			...
			if(getperm('developer', doc.title)) {
				data = `
					<div style="border-width: 5px 1px 1px; border-style: solid; border-color: #a188f1 #bcbcbc #bcbcbc; padding: 10px; margin-bottom: 10px;" onmouseover="this.style.borderTopColor=\'#c6b7f6\';" onmouseout="this.style.borderTopColor=\'#a188f1\';">
						<span${ver('4.13.0') ? '' : ' style="font-size: 14pt;"'}>이 사용자는 ${config.getString('wiki.site_name')}의 개발자입니다.</span>
					</div>
				` + data;
				}
			if(getperm('owner', doc.title)) {
				data = `
					<div style="border-width: 5px 1px 1px; border-style: solid; border-color: #a188f1 #bcbcbc #bcbcbc; padding: 10px; margin-bottom: 10px;" onmouseover="this.style.borderTopColor=\'#c6b7f6\';" onmouseout="this.style.borderTopColor=\'#a188f1\';">
						<span${ver('4.13.0') ? '' : ' style="font-size: 14pt;"'}>이 사용자는 ${config.getString('wiki.site_name')}의 소유자입니다.</span>
					</div>
				` + data;
				}
			if(getperm('official_admin', doc.title)) {
				data = `
					<div style="border-width: 5px 1px 1px; border-style: solid; border-color: #a188f1 #bcbcbc #bcbcbc; padding: 10px; margin-bottom: 10px;" onmouseover="this.style.borderTopColor=\'#c6b7f6\';" onmouseout="this.style.borderTopColor=\'#a188f1\';">
						<span${ver('4.13.0') ? '' : ' style="font-size: 14pt;"'}>이 사용자는 ${config.getString('wiki.site_name')}의 정식 관리자입니다.</span>
					</div>
				` + data;
				}
		}
